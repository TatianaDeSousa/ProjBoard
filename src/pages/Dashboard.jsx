...
import { 
  ...
  LayoutGrid, StickyNote, CheckCircle2, HeartPulse, Zap, Sparkles, Clock, Target, Flame
} from 'lucide-react';
...
const HealthBadge = ({ score }) => {
  ...
  let label = "En bonne santé";
  if (score < 40) { color = "bg-red-100 text-red-600 animate-pulse"; label = "En danger"; }
  else if (score < 70) { color = "bg-amber-100 text-amber-600"; label = "À surveiller"; }
  
  return (
    <Badge className={cn("font-black text-[9px] uppercase tracking-widest border-none px-3 h-6", color)}>
       {label} • {score}
    </Badge>
  );
};
...
const QuickNotes = () => {
  ...
  return (
    <Card className="p-8 border-none shadow-2xl bg-white rounded-[3rem] ring-1 ring-black/5 flex flex-col h-full group">
       ...
       <textarea className="flex-1 w-full bg-slate-50 border-none rounded-2xl p-6 font-bold text-slate-600 focus:ring-1 focus:ring-amber-200 resize-none min-h-[220px] shadow-inner transition-all focus:bg-white text-md" placeholder="Notes rapides..." value={note} onChange={(e) => setNote(e.target.value)} />
    </Card>
  );
};

const ProjectList = ({ projects, title, icon: Icon, onShare }) => {
  // TRI AUTOMATIQUE (DANGER EN HAUT)
        ...
        {sortedProjects.map(project => {
          ...
          return (
            <Link key={project.id} to={`/project/${project.id}`} className={cn("group transition-all duration-500 hover:-translate-y-2 relative", isDanger && "z-20 scale-105")}>
              <Card className={cn(
                "p-10 hover:shadow-[0_40px_60px_-15px_rgba(99,102,241,0.15)] transition-all border-none h-full flex flex-col justify-between bg-white rounded-[3.5rem] ring-1 ring-black/5 overflow-hidden",
                ...
                <div className="relative z-10">
                  ...
                  <div className="space-y-4 mb-6">
                    ...
                    <div className="h-3 w-full bg-slate-50 rounded-full p-0.5 overflow-hidden shadow-inner flex ring-1 ring-slate-100">
                      <div className="h-full rounded-full gradient-primary shadow-lg transition-all duration-[1000ms]" style={{ width: `${progress}%` }} />
                    </div>
                  </div>
                </div>

                {/* MINI BARRE DE SANTÉ v2.7 */}
                <div className="mt-4 pt-4 border-t border-slate-50">
                    <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                       <div className={cn("h-full transition-all duration-1000", score >= 70 ? "bg-emerald-500" : score >= 40 ? "bg-amber-500" : "bg-red-500")} style={{ width: `${score}%` }} />
                    </div>
                </div>
...
const Dashboard = () => {
  ...
  // LOGIQUE FOCUS SANTÉ (Top 3 scores bas)
  const healthFocus = [...projects]
    .filter(p => p.status !== 'done')
    .sort((a, b) => calculateHealthScore(a) - calculateHealthScore(b))
    .slice(0, 3);

  // LOGIQUE FOCUS DU JOUR (Score < 40 ET Deadline < 3j)
    ...
    <div className="container mx-auto px-6 py-12 animate-in max-w-7xl pb-40">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-20 gap-10 px-4">
        ...
        <div className="flex flex-wrap gap-6 items-center">
          <Link to="/project/new"><Button className="h-20 px-10 font-black gradient-primary border-none shadow-premium text-white rounded-[1.8rem] hover:scale-105 transition-all active:scale-95 gap-4 text-xl group relative overflow-hidden"><Plus size={32} /><span>Nouveau Projet</span></Button></Link>
      ...
      {dayFocus.length > 0 && (
        <div className="mb-24 px-4">
           <div className="flex items-center gap-3 mb-8"><Flame size={24} className="text-red-500 animate-bounce" /><h3 className="text-xl font-black uppercase tracking-[0.3em] text-red-500">Urgence : Focus du Jour</h3></div>
                ...
                <Link key={p.id} to={`/project/${p.id}`}>
                  <Card className="p-8 border-none bg-red-500 text-white rounded-[2.5rem] shadow-2xl shadow-red-500/30 ring-4 ring-red-100 flex items-center justify-between hover:scale-[1.02] transition-all">
                     <div className="flex items-center gap-6">
                        ...
                        <div>
                           <p className="text-2xl font-black tracking-tighter">{p.name}</p>
                           <p className="text-xs font-black uppercase tracking-widest opacity-70">Deadline dans moins de 3 jours • Score {calculateHealthScore(p)}</p>
                        </div>
                     </div>
      ...
      {/* SECTION SANTÉ DES PROJETS v2.7 */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-24 px-4">
         <div className="lg:col-span-12 xl:col-span-8 flex flex-col">
            <div className="flex items-center justify-between mb-8">
               <h3 className="text-xs font-black uppercase tracking-[0.4em] text-slate-300 flex items-center gap-3"><HeartPulse size={18} className="text-primary" /> Santé Critique (Priorité Tri)</h3>
               <Link to="/folders" className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-primary transition-colors">Voir l'état global</Link>
            </div>
               ...
               {healthFocus.map(p => {
                 ...
                 return (
                   <Card key={p.id} className={cn(
                     "p-6 border-none shadow-xl rounded-[2rem] flex flex-col justify-between transition-all hover:scale-105",
                     score < 40 ? "bg-red-50 ring-2 ring-red-100" : "bg-white"
                   )}>
                      <div>
                         <p className="text-[9px] font-black uppercase text-slate-400 tracking-[0.3em] mb-4">{p.client}</p>
                         <h4 className="text-lg font-black tracking-tighter text-slate-900 mb-2 truncate">{p.name}</h4>
                         <p className={cn("text-5xl font-black tracking-tighter mb-4", score < 40 ? "text-red-500" : "text-amber-500")}>{score}%</p>
                      </div>
                      <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                         <div className={cn("h-full transition-all duration-[2000ms]", score < 40 ? "bg-red-500" : "bg-amber-500")} style={{ width: `${score}%` }} />
                      </div>
                   </Card>
                 );
               })}
         ...
         <div className="xl:col-span-4 flex flex-col h-full"><h3 className="text-xs font-black uppercase tracking-[0.4em] text-slate-300 mb-8 flex items-center gap-3"><StickyNote size={18} /> Notes Tactiques</h3><QuickNotes /></div>
      </div>

      <div className="px-4 space-y-24">
         <div className="relative"><Search className="absolute left-8 top-1/2 -translate-y-1/2 text-slate-300" size={24} /><Input placeholder="Filtre Projet..." className="pl-20 h-20 text-xl bg-white shadow-2xl border-none rounded-[3rem] ring-1 ring-black/5 font-black" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
         <ProjectList projects={filteredProjects} title="Base de Données Projets" icon={LayoutGrid} onShare={handleShare} />
         <div className="flex flex-col"><h3 className="text-xs font-black uppercase tracking-[0.4em] text-slate-300 mb-8 flex items-center gap-3"><Calendar size={18} /> Flux Temporel</h3><VisualCalendar projects={projects} /></div>
      </div>
    </div>
  );
};
