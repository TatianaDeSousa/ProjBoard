...
const ProjectDetail = () => {
  ...
  // VARIABLES MANQUANTES À DÉCLARER (CAUSE DU CRASH)
  ...
  const handleSyncShare = async () => {
    ...
    } catch (err) { alert("Le service Cloud est inaccessible."); }
    setSyncBusy(false);
  };
    ...
    <div className="container mx-auto px-6 py-10 max-w-7xl animate-in pb-40">
      ...
      <div className="flex flex-col lg:flex-row gap-12 items-start">
        <div className="flex-1 space-y-10 w-full">
           <Card className={cn(
             "p-12 border-none shadow-[0_50px_100px_-20px_rgba(0,0,0,0.05)] bg-white rounded-[4rem] relative overflow-hidden ring-1 ring-slate-100",
             isDanger ? "ring-2 ring-red-500 shadow-xl" : ""
           )}>
              ...
              <div className="flex flex-col md:flex-row justify-between items-start mb-12 relative z-10 gap-8">
                <div className="flex-1">
                   <div className="flex items-center gap-3 mb-6">
                      ...
                      <Badge className="bg-slate-50 text-slate-300 border-none font-black text-[10px] uppercase h-8 px-4 rounded-full italic shadow-sm">ID: {String(project.id).slice(0,8)}</Badge>
                   </div>
                   <h1 className="text-7xl font-black tracking-tighter text-slate-900 mb-4 leading-none">{project.name}</h1>
                   <p className="text-xl text-slate-400 font-extrabold italic uppercase tracking-widest text-[10px] opacity-60">Mission de {project.client}</p>
                </div>

                <div className="flex flex-col gap-4 min-w-[320px]">
                   <Button onClick={handleSyncShare} disabled={syncBusy} className="h-20 px-10 font-black gradient-primary border-none text-white rounded-[2rem] shadow-[0_20px_40px_-10px_rgba(99,102,241,0.4)] hover:scale-105 active:scale-95 transition-all flex gap-4 text-2xl group overflow-hidden relative">
                      {syncBusy ? <RefreshCcw size={32} className="animate-spin" /> : <><Share2 size={32} /> Accès Client</>}
                   </Button>
                   
                   {shareLink && (
                     <div className="p-6 bg-slate-950 border-none rounded-[2rem] animate-in slide-in-from-top text-center shadow-2xl relative overflow-hidden">
                        <div className="absolute inset-0 bg-primary/10 opacity-50 blur-xl" />
                        <p className="text-[10px] font-black uppercase text-primary tracking-widest mb-2 relative z-10">Lien Client Sécurisé ! ✅</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-10 relative z-10 pt-10 border-t border-slate-50">
                 <div className="md:col-span-2 space-y-6">
                    <p className="text-[11px] font-black uppercase text-slate-400 tracking-widest">Score Vitalité</p>
                    <div className="flex items-center gap-8">
                       ...
                       <div className="flex-1 space-y-2">
                          <div className="h-4 w-full bg-slate-50 rounded-full overflow-hidden p-0.5 relative shadow-inner ring-1 ring-slate-100">
                             <div className={cn("h-full rounded-full transition-all duration-[2000ms]", score >= 70 ? "bg-emerald-500" : score >= 40 ? "bg-amber-500" : "bg-red-500")} style={{ width: `${score}%` }} />
                          </div>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="space-y-3 md:pl-10 md:border-l border-slate-50">
                    <p className="text-[11px] font-black uppercase text-slate-400 tracking-widest">Retours</p>
              ...
              <div className="grid gap-6">
                      ...
                      <div className="flex items-center gap-8">
                         ...
                         <div>
                            <div className="flex items-center gap-4 mb-2">
                               ...
                               {m.status === 'doing' && <Badge className="bg-amber-50 text-amber-600 border-none font-black text-[9px] h-6 px-3 rounded-full">EN COURS</Badge>}
                            </div>
                 ...
                 <div className="p-10 border-4 border-dashed border-slate-100 bg-slate-50/10 rounded-[4rem] text-center">
                    <form onSubmit={(e) => { e.preventDefault(); if(!newMilestoneName) return; addMilestone(id, { name: newMilestoneName, dueDate: newMilestoneDate ? new Date(newMilestoneDate).toISOString() : null }); setNewMilestoneName(''); setNewMilestoneDate(''); }} className="flex flex-col md:flex-row gap-6">
                       <Input placeholder="Titre de l'étape..." value={newMilestoneName} onChange={e => setNewMilestoneName(e.target.value)} className="h-20 font-black border-none shadow-inner rounded-[2rem] bg-white text-xl" />
                       <Input type="date" value={newMilestoneDate} onChange={e => setNewMilestoneDate(e.target.value)} className="h-20 font-black border-none shadow-inner rounded-[2rem] bg-white text-xl md:w-64" />
                       <Button type="submit" className="h-20 px-12 gradient-primary border-none text-white font-black rounded-[2rem] shadow-xl hover:scale-105 transition-all text-xl">Ajouter</Button>
                    </form>
                 </div>
              </div>
           </div>
        ...
        <div className="w-full lg:w-[450px] space-y-12">
           <Card className="p-12 border-none shadow-2xl bg-white rounded-[4rem] flex flex-col group transition-all duration-700 hover:ring-primary/20">
              ...
              <textarea className="w-full bg-slate-50 border-none rounded-[3rem] p-10 font-bold text-slate-700 focus:ring-2 focus:ring-primary/10 resize-none min-h-[500px] shadow-inner transition-all focus:bg-white text-xl leading-relaxed" placeholder="Notes..." value={notes} onChange={(e) => setNotes(e.target.value)} />
           </Card>
